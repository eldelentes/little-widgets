<template>
  <div class="lw-card" v-bind:style="{
    ...
    <div class="lw-card__content">
        <h4 class="lw-card__subtitle" v-bind:style="{
          textTransform: subtitleStyle,
          color: subtitleColor,
        }">{{ widgetSubtitle }}</h4>
        <h2 class="lw-card__title" v-bind:style="{
          fontFamily: titleFontFamily,
          color: titleColor,
        }">{{  widgetTitle }}</h2>
    </div>
    <div class="lw-card__action">
        <a class="lw-card__button" href="#" v-bind:style="{
          background: buttonBackground,
          color: buttonColor,
          borderRadius: buttonBorderRadius + 'em',
        }">{{ widgetButtonText }}</a>
    </div>
  </div>
</template>
    ...
    beforeCreate () {
        db.collection('widgets').where('widgetData.wid', '==',  this.$route.params.widget).get().then((querySnapshot) => {   
            querySnapshot.forEach((doc) => {
                ...
                this.cardBoxShadow = "0 " + widget.cardBoxShadowY + "px " + widget.cardBoxShadowSize + "px rgba(0,0,0, " + widget.cardBoxShadowOpacity + ")";
                this.widgetSubtitle = widget.subtitle;
                this.widgetTitle = widget.title;
                // this.widgetButtonText = buttonText;
                this.subtitleStyle = widget.subtitleStyle;
                this.subtitleColor = widget.subtitleColor;
                this.titleFontFamily = widget.titleFontFamily;
                this.titleColor = widget.titleColor;
                this.buttonBackground = widget.buttonBackground;
                this.buttonColor = widget.buttonColor;
                this.buttonBorderRadius = widget.buttonBorderRadius;
            });
        });

        db.collection('widgets').get().then((querySnapshot) => {   
            querySnapshot.forEach((doc) => {
